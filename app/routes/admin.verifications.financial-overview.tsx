...
export const loader: LoaderFunction = async ({ request }) => {
  ...
  const from = fromParam 
    ? new Date(fromParam.slice(0, 4), fromParam.slice(4, 6) - 1, fromParam.slice(6, 8)) // Format YYYYMMDD
    : new Date(currentYear, 0, 1); // 1 januari innevarande år
  const to = toParam 
    ? new Date(toParam.slice(0, 4), toParam.slice(4, 6) - 1, toParam.slice(6, 8), 23, 59, 59, 999) // Format YYYYMMDD
    : new Date(currentYear, 11, 31, 23, 59, 59, 999); // 31 december innevarande år
...
const filterAccounts = (type: ReportType) => accounts.filter((account) => account.reportType === type);

const sumAccounts = (verifications, accounts) => {
    return accounts.reduce((total, account) => {
      const accountTotal = verifications.reduce((acc, v) => {
        const entries = v.journalEntries.filter((entry) => entry.account === account);
        const accountSum = entries.reduce((sum, entry) => sum + (entry.debit || 0) - (entry.credit || 0), 0);
...
const FinancialReportSection = ({ title, accounts, verifications }) => (
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {accounts.map((account) => (
            <tr key={account.value}>
              <td className="px-4 py-3 whitespace-nowrap text-sm text-gray-700">{account.label}</td>
          ...
          <tr className="font-bold bg-gray-100">
            ...
            <td className="px-4 py-3 text-right">
              {sumAccounts(verifications, accounts.map((a) => a.value))} SEK
            </td>
          </tr>
        </tbody>
      </table>
  ...
  function formatDateRange(startDate, endDate) {
    ...
    const end = new Date(endDate).getFullYear() > new Date(startDate).getFullYear()
      ? "2024-12-31"
            ...
            <div className="bg-white px-6 py-5 sm:p-6 sm:pb-4">
              <div className="sm:flex sm:items-start ">
                <div className="text-center sm:ml-4 sm:text-left w-full">
                  ...
                  <div className="mt-6">
                    {/* Balansräkning */}
                    <FinancialReportSection title="Tillgångar" accounts={filterAccounts(ReportType.BALANCE)} verifications={verifications} />
                    <FinancialReportSection title="Skulder" accounts={filterAccounts(ReportType.LIABILITIES)} verifications={verifications} />
  
                    {/* Resultaträkning */}
                    <FinancialReportSection title="Intäkter" accounts={filterAccounts(ReportType.INCOME)} verifications={verifications} />
                    <FinancialReportSection title="Kostnader" accounts={filterAccounts(ReportType.EXPENSE)} verifications={verifications} />
                  </div>
                </div>
              </div>
            </div>
